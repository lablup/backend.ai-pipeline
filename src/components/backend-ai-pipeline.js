...
/**
 ...
 `backend-ai-pipeline` is a shell of Backend.AI End-to-end Pipeline (web).
 ...
 */
class BackendAiPipeline extends connect(store)(LitElement) {
  constructor() {
    super();
  ...
  updated(changedProps) {
    if (changedProps.has('_page')) {
      let view = this._page;
      // load data for view
      if (['summary', 'job', 'data'].includes(view) !== true) { // Fallback for Windows OS
        view = view.split(/[\/]+/).pop();
        this._page = view;
      }
      switch (view) {
  ...
  async logout() {
    ...
    for (let i = 0; i < keys.length; i++) {
      const key = keys[i];
      if (/^(BackendAiPipeline\.)/.test(key)) localStorage.removeItem(key);
    }
    if (window.isElectron) {
      this.shadowRoot.querySelector('#login-panel').login();
    } else {
...
customElements.define(BackendAiPipeline.is, BackendAiPipeline);
